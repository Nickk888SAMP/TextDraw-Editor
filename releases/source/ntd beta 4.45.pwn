...
//SETTINGS
#define SCRIPT_VERSION 					"v4.45 Beta"
#define SCRIPT_VERSION_CHECK 			"4.45"
...
#define DIALOG_RENAMEPROJECT 542
...
public OnFilterScriptInit()
{
	...
	{
		ScriptActive = true;
		printf("\n[NTD PL]Edytor TextDrawow od Nickk888 %s zostal pomyslnie zinicjalizowany!\n", SCRIPT_VERSION);
		printf("[NTD ENG]The TextDraw editor from Nickk888 %s has been successfully initialized!\n", SCRIPT_VERSION);
	}
	else
	{
		ScriptActive = false;
		...
		printf("\n[NTD ERROR ENG]Could not find folder or folders!");
		printf("[NTD ERROR ENG]Please create 'NTD' folder inside 'scriptfiles'!");
		printf("[NTD ERROR ENG]Within 'NTD' you should create the 'Exports' and 'Projects' folder!");
		printf("[NTD ERROR ENG]The script will be blocked!\n");
		return 1;
	}
...
CMD:ntd(playerid, params[])
{
		...
		if(ScriptActive)
		{
			if(NTDPlayer[InEditor] == false)
			{
				...
				if(!strcmp(EditorVersion, SCRIPT_VERSION_CHECK) && !isnull(EditorVersion))
				{
					...
					else if(Lang(LANG_ENGLISH)) ShowInfo(playerid, "{FFFFFF}To leave the Editor, use this command {00FFFF}/NTD");
				}
				else
				{
					...
					else if(Lang(LANG_ENGLISH)) ShowPlayerDialog(playerid, DIALOG_SETTINGRESET, DIALOG_STYLE_MSGBOX, CAPTION_TEXT"New Version", "{FF0000}The script version is different from the configuration file!\n{FFFFFF}Would you like to restore the default settings so that all functions work correctly?", "Yes", "No");
				}	
				for(new i; i < 15; i++)
			...
			else
			{
				if(NTDPlayer[playerIDInEditor] == playerid)
				{
					...
					else if(Lang(LANG_ENGLISH)) SendClientMessage(playerid, -1, "{FF8040}NTD: {FFFFFF}The editor has been disabled.");
				...
				else 
				{
					...
					if(Lang(LANG_ENGLISH) || Lang(LANG_NONE)) SendClientMessage(playerid, -1, "{FF8040}NTD: {FF0000}Another player is currently using the editor!");
				}
			}
		}
		else 
	...
	else
	{
		...
		if(Lang(LANG_ENGLISH) || Lang(LANG_NONE)) SendClientMessage(playerid, -1, "{FF8040}NTD: {FF0000}You are not authorized to use this command! Please Login to RCON.");
	}
	return 1;
}
	...
	if(ScriptActive && NTDPlayer[InEditor])
	{
		...
		if(dialogid == DIALOG_SPRITES1)
		{
			if(response)
			{
				if(listitem == 0) //Wpisz recznie
				{
					...
					else if(Lang(LANG_ENGLISH)) ShowPlayerDialog(playerid, DIALOG_TEKST, DIALOG_STYLE_INPUT, CAPTION_TEXT"Change Sprite", "{FFFFFF}Enter the name of the library and sprite name.\nExample: {FF8040}hud:radar_impound", "Confirm", "Back");
				}
		...
		if(dialogid == DIALOG_SETTINGRESET)
		{
			if(response)
			{
				...
				else if(Lang(LANG_ENGLISH)) ShowInfo(playerid, "{00FF00}Settings have been restored!");
			}
			else
			{
				EditorVersion = SCRIPT_VERSION_CHECK;
			...
			if(response)
			{
				...
				if(listitem == 1) //Zmien kolor najechania
				{
					...
					else if(Lang(LANG_ENGLISH)) 
					{
						...
						ShowPlayerDialog(playerid, DIALOG_SETTINGSCOLOR, DIALOG_STYLE_LIST, CAPTION_TEXT"Change overriding colors", longstring, "Confirm", "Back");
					}
				}
		...
		if(dialogid == DIALOG_PREVIEWMODEL)
		{
			if(response)
			{
				if(listitem == 0) //Zmien Model
				{
					...
					else if(Lang(LANG_ENGLISH)) ShowPlayerDialog(playerid, DIALOG_PREVIEWMODEL1, DIALOG_STYLE_INPUT, CAPTION_TEXT"Preview Model", "{FFFFFF}Enter your new Skin model ID, Vehicle Model ID or object Model ID.", "Confirm", "Back");
				}
				if(listitem > 0)
				{
			...
			if(response)
			{
				if(IsNumeric(inputtext))
				{
					...
					else
					{
						...
						else if(Lang(LANG_ENGLISH)) ShowPlayerDialog(playerid, DIALOG_COLOR7, DIALOG_STYLE_INPUT, CAPTION_TEXT"Change color", "{FFFFFF}Enter the fourth value for opacity\n{FFFFFF}Range of 0-255.", "Next", "Back");
					}
				}
				else
				{
			...
			if(response)
			{
				if(IsNumeric(inputtext))
				{
					...
					else
					{
						...
						else if(Lang(LANG_ENGLISH)) ShowPlayerDialog(playerid, DIALOG_COLOR6, DIALOG_STYLE_INPUT, CAPTION_TEXT"Change color", "{FFFFFF}Enter the value for {0000FF}BLUE\n{FFFFFF}Range of 0-255.", "Next", "Back");
					}
				}
				else
				{
			...
			if(response)
			{
				if(IsNumeric(inputtext))
				{
					...
					else
					{
						...
						else if(Lang(LANG_ENGLISH)) ShowPlayerDialog(playerid, DIALOG_COLOR5, DIALOG_STYLE_INPUT, CAPTION_TEXT"Change color", "{FFFFFF}Enter the value for {00FF00}GREEN\n{FFFFFF}Range of 0-255.", "Next", "Back");
					}
				}
				else
				{
			...
			if(response)
			{
				if(IsNumeric(inputtext))
				{
					...
					else
					{
						...
						else if(Lang(LANG_ENGLISH)) ShowPlayerDialog(playerid, DIALOG_COLOR4, DIALOG_STYLE_INPUT, CAPTION_TEXT"Change color", "{FFFFFF}Enter the value for {FF0000}RED\n{FFFFFF}Range of 0-255.", "Next", "Back");
					}
				}
				else
				{
			...
			if(response)
			{
				if(strlen(inputtext) > 0 && strlen(inputtext) <= 30)
				{
					if(IsValidString(inputtext))
					{
						...
						{
							format(NTD[NTDPlayer[ChoosenTDID]][tdvarname], 30, inputtext);
							if(Lang(LANG_POLISH)) format(string, sizeof string, "{FFFFFF}Nazwa zmiennej zostala zmieniona!\nNowa nazwa Parametru: {00FF00}%s\n{FFFFFF}Tekst TextDrawa: {00FF00}%s", NTD[NTDPlayer[ChoosenTDID]][tdvarname], NTD[NTDPlayer[ChoosenTDID]][tdstring]);
							else if(Lang(LANG_ENGLISH)) format(string, sizeof string, "{FFFFFF}Variable name has been changed!\nNew variable name: {00FF00}%s\n{FFFFFF}TextDraw string: {00FF00}%s", NTD[NTDPlayer[ChoosenTDID]][tdvarname], NTD[NTDPlayer[ChoosenTDID]][tdstring]);
						
							ShowInfo(playerid, string);
						}
						else
						{
							if(Lang(LANG_POLISH)) ShowInfo(playerid, "{FFFFFF}Taka nazwa zmiennej zostala juz uzywana!");
							else if(Lang(LANG_ENGLISH)) ShowInfo(playerid, "{FFFFFF}This variable name has been already used!");
						}
					}
					else
					{
						if(Lang(LANG_POLISH)) ShowInfo(playerid, "{FFFFFF}Nazwa zmiennej nie moze zawierac specjalnych znakow oraz przerw!");
						else if(Lang(LANG_ENGLISH)) ShowInfo(playerid, "{FFFFFF}You can't use special letters and spaces in variables!");
					}
				}
				else
				{
					if(Lang(LANG_POLISH)) ShowInfo(playerid, "{FFFFFF}Nazwa zmiennej musi byc w obrebiu 1-30 znakow!");
					else if(Lang(LANG_ENGLISH)) ShowInfo(playerid, "{FFFFFF}Variable name has to be in range of 1-30 letters!");
				}
			}
			else ShowEditorEx(playerid);
		}
			...
			if(response)
			{
				...
				else if(listitem == 1) //Zklonuj
				{
					...
					else
					{
						...
						else if(Lang(LANG_ENGLISH)) ShowInfo(playerid, "You have reached the maximum amount of textdraws!");
					}
				}
				else if(listitem == 2) //Zmien nazwe parametru
				{
					...
					else if(Lang(LANG_ENGLISH)) format(string, sizeof string, "{FFFFFF}Enter the new variable name\nActual variable name: {00FF00}%s\n{FFFFFF}TextDraw string: {00FF00}%s", NTD[NTDPlayer[ChoosenTDID]][tdvarname], NTD[NTDPlayer[ChoosenTDID]][tdstring]);
					...
					else if(Lang(LANG_ENGLISH)) ShowPlayerDialog(playerid, DIALOG_CHANGEPARAMNAME, DIALOG_STYLE_INPUT, CAPTION_TEXT"Change variable name", string, "Confirm", "Back");
				}
				else if(listitem == 3) //Usun
				{
					...
					else if(Lang(LANG_ENGLISH)) format(string, sizeof string, "{FF0000}Are you sure you want to remove this TextDraw?\n{FFFFFF}String: {00FF00}\x22%s\x22", formatedtd);
					if(Lang(LANG_POLISH)) ShowPlayerDialog(playerid, DIALOG_DELETETD, DIALOG_STYLE_MSGBOX, CAPTION_TEXT"Usuwanie", string, "Tak", "Nie");
					else if(Lang(LANG_ENGLISH)) ShowPlayerDialog(playerid, DIALOG_DELETETD, DIALOG_STYLE_MSGBOX, CAPTION_TEXT"Removing", string, "Yes", "No");
				}
		...
		if(dialogid == DIALOG_DELETETD)
		{
			if(response)
			{
				...
				else if(Lang(LANG_ENGLISH)) format(string, sizeof string, "~r~REMOVED_ID_%i", NTDPlayer[ChoosenTDID]);
				GameTextForPlayer(playerid, string, 5000, 6);
				PlayerPlaySound(playerid, 1083, 0.0, 0.0, 0.0);
				ShowEditorEx(playerid, true);
			}
		...
		if(dialogid == DIALOG_RENAMEPROJECT)
		{
			if(response)
			{
				new oldproname[60];
				format(oldproname, 60, NTDPlayer[ProjectName]);
				new changepn = RenameProject(oldproname, inputtext);
				if(changepn == 1)
				{
					if(Lang(LANG_POLISH)) format(string, sizeof string, "{FFFFFF}Nazwa projektu zostala pomyslnie zmieniona!\nStara nazwa projektu: {00FF00}\x22%s\x22\n{FFFFFF}Nowa nazwa projektu: {00FF00}\x22%s\x22", oldproname, inputtext);
					else if(Lang(LANG_ENGLISH)) format(string, sizeof string, "{FFFFFF}The name of the project has been changed successfully!\nOld project name: {00FF00}\x22%s\x22\n{FFFFFF}New project name: {00FF00}\x22%s\x22", oldproname, inputtext);
					ShowInfo(playerid, string);
				}
				else if(changepn == 2)
				{
					if(Lang(LANG_POLISH)) ShowInfo(playerid, "{FF0000}Nazwa jest za krotka, za dluga lub posiada niepoprawne znaki!");
					else if(Lang(LANG_ENGLISH)) ShowInfo(playerid, "{FF0000}The name is too short, too long, or has invalid characters!");
				}
				else if(changepn == 3)
				{
					if(Lang(LANG_POLISH)) ShowInfo(playerid, "{FF0000}Taki projekt juz istnieje!");
					else if(Lang(LANG_ENGLISH)) ShowInfo(playerid, "{FF0000}Such a project already exists!");
				}
				else if(changepn == -1)
				{
					if(Lang(LANG_POLISH)) ShowInfo(playerid, "{FF0000}Podczas zmiany nazwu projektu wystapil problem!");
					else if(Lang(LANG_ENGLISH)) ShowInfo(playerid, "{FF0000}An error has occured while renaming this project!");
				}
			}
			else ShowEditorEx(playerid);
		}
		if(dialogid == DIALOG_OPEN2)
		{
			if(response)
			{
				...
				else if(listitem == 1) //Zmien nazwe
				{
					if(Lang(LANG_POLISH)) format(string, sizeof string, "{FFFFFF}Wpisz nowa nazwe tego projektu!\nAktualna nazwa: {00FF00}\x22%s\x22",NTDPlayer[ProjectName]);
					else if(Lang(LANG_ENGLISH)) format(string, sizeof string, "{FFFFFF}Enter a new name for this project!\nActual name: {00FF00}\x22%s\x22", NTDPlayer[ProjectName]);
					if(Lang(LANG_POLISH)) ShowPlayerDialog(playerid, DIALOG_RENAMEPROJECT, DIALOG_STYLE_INPUT, CAPTION_TEXT"Zmiana nazwy projektu", string, "Potwierdz", "Wroc");
					else if(Lang(LANG_ENGLISH)) ShowPlayerDialog(playerid, DIALOG_RENAMEPROJECT, DIALOG_STYLE_INPUT, CAPTION_TEXT"Rename the project", string, "Confirm", "Back");
					PlayerSelectTD(playerid, false);
				}
				else if(listitem == 2) //Usun
				{
					...
					else if(Lang(LANG_ENGLISH)) ShowPlayerDialog(playerid, DIALOG_DELETEPROJECT, DIALOG_STYLE_MSGBOX, CAPTION_TEXT"Deleting", "{FF0000}Are you sure you want to remove this Project?", "Yes", "No");
				}
			}
			else ShowEditorEx(playerid);
		}
	...
	if(ScriptActive && NTDPlayer[InEditor])
	{
		...
		else if(clickedid == B_Tekst)
		{
			...
			else if(NTD[tdid][Font] == 4) 
			{
				...
				else if(Lang(LANG_ENGLISH)) ShowPlayerDialog(playerid, DIALOG_SPRITES1, DIALOG_STYLE_LIST, CAPTION_TEXT"Change sprite", "{FF8040}Enter manually\n{FF5500}Select from library", "Confirm", "Back");
			}
			else if(NTD[tdid][Font] == 5) 
			{
				if(Lang(LANG_POLISH)) ShowInfo(playerid, "{FF0000}Nie mozna zmienic Tekst podgladu modelu...");
				else if(Lang(LANG_ENGLISH)) ShowInfo(playerid, "{FF0000}You can't change the preview model's string...");
			}
			PlayerSelectTD(playerid, false);
			return 1;
		}
		...
		else if(clickedid == B_NewProject)
		{
			...
			else if(Lang(LANG_ENGLISH)) ShowPlayerDialog(playerid, DIALOG_NEW, DIALOG_STYLE_INPUT, CAPTION_TEXT"New Project", "{FFFFFF}Enter your new project name...", "Create", "Back");
			PlayerSelectTD(playerid, false);
			return 1;
		}
...
stock RenameProject(projectname[], newprojectname[])
{
	...
	format(NTDPlayer[ProjectName], 35, newprojectname);
	SaveProject();
	return 1;
}
...
stock bool:VariableExists(string[])
{
	new tda = GetTDPoolSize();
	for(new i; i <= tda; i++)
		if(!strcmp(NTD[i][tdvarname], string, false) && !isnull(NTD[i][tdvarname]) && NTD[i][tdCreated]) return true;
	return false;
}
			...
			if(file)
			{
				...
				if(exporttype == 0) //RAW EXPORT
				{
					...
					for(new i; i < tda; i++)
					{
						...
						{
							format(string, sizeof string, "%s = TextDrawCreate(%f, %f, \x22%s\x22);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][PosX], NTD[TDAdress[i]][PosY], NTD[TDAdress[i]][tdstring]);
							fwrite(file, string);
							format(string, sizeof string, "TextDrawFont(%s, %i);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][Font]);
							fwrite(file, string);
							format(string, sizeof string, "TextDrawLetterSize(%s, %f, %f);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][LetterSizeX], NTD[TDAdress[i]][LetterSizeY]);
							fwrite(file, string);
							format(string, sizeof string, "TextDrawTextSize(%s, %f, %f);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][BoxSizeX], NTD[TDAdress[i]][BoxSizeY]);
							fwrite(file, string);
							format(string, sizeof string, "TextDrawSetOutline(%s, %i);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][OutlineSize]);
							fwrite(file, string);
							format(string, sizeof string, "TextDrawSetShadow(%s, %i);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][ShadowSize]);
							fwrite(file, string);
							format(string, sizeof string, "TextDrawAlignment(%s, %i);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][Alignment]);
							fwrite(file, string);
							format(string, sizeof string, "TextDrawColor(%s, %i);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][Color]);
							fwrite(file, string);
							format(string, sizeof string, "TextDrawBackgroundColor(%s, %i);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][BGColor]);
							fwrite(file, string);
							format(string, sizeof string, "TextDrawBoxColor(%s, %i);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][BoxColor]);
							fwrite(file, string);
							format(string, sizeof string, "TextDrawUseBox(%s, %i);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][UseBox]);
							fwrite(file, string);
							format(string, sizeof string, "TextDrawSetProportional(%s, %i);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][Proportional]);
							fwrite(file, string);
							format(string, sizeof string, "TextDrawSetSelectable(%s, %i);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][Selectable]);
							fwrite(file, string);
							if(NTD[TDAdress[i]][Font] == TEXT_DRAW_FONT_MODEL_PREVIEW)
							{
								format(string, sizeof string, "TextDrawSetPreviewModel(%s, %i);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][PrevModel]);
								fwrite(file, string);
								format(string, sizeof string, "TextDrawSetPreviewRot(%s, %f, %f, %f, %f);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][PrevRotX], NTD[TDAdress[i]][PrevRotY], NTD[TDAdress[i]][PrevRotZ], NTD[TDAdress[i]][PrevRotZoom]);
								fwrite(file, string);
								format(string, sizeof string, "TextDrawSetPreviewVehCol(%s, %i, %i);\n", NTD[TDAdress[i]][tdvarname], NTD[TDAdress[i]][PrevModelC1], NTD[TDAdress[i]][PrevModelC2]);
								fwrite(file, string);
...
stock OpenTDDialog(playerid, pageid)
{
	...
	else if(Lang(LANG_ENGLISH)) longstring = "{F2E26F}ID/String\t{FF80FF}Variable name\n";
...
stock OpenProjectDialog(playerid, pageid)
{
	...
	if(projamount > 0)
	{
		if(Lang(LANG_POLISH)) strcat(longstring, "{F2E26F}Nazwa\t{F2E26F}Ilosc TD\t{F2E26F}Ostatnio zmodyfikowano\n");
		else if(Lang(LANG_ENGLISH)) strcat(longstring, "{F2E26F}Name\t{F2E26F}TD amount\t{F2E26F}Recently modified\n");
...
stock SaveProject()
{
	...
	if(dfile_FileExists(file) && NTDPlayer[ProjectOpened])
	{
		dfile_Open(file);
		for(new i; i < MAX_TDS; i++)
		{
			if(NTD[i][tdCreated])
			{
				...
				strcat(longstring, stringex);
				format(stringex, sizeof stringex, "%i %i %i %s", NTD[i][ColorA], NTD[i][BGColorA], NTD[i][BoxColorA], NTD[i][tdvarname]);
				strcat(longstring, stringex);
	...
	if(dfile_FileExists(string))
	{
		...
		for(new i; i < MAX_TDS; i++)
		{
			...
			if(strlen(dfile_ReadString(string)) > 0)
			{
				...
				NTD[i][ColorA], NTD[i][BGColorA], NTD[i][BoxColorA], NTD[i][tdvarname]);
				
				if(strlen(NTD[i][tdvarname]) == 0)
					format(NTD[i][tdvarname], 35, "textdraw_%i", i);
